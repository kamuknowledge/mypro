                    ...
                    $.each(this.split(','), function(j, buttonName) {
                        if (buttonName == 'title') {
                            ...
                            if (buttonClick) {
                                ...
                                var button = $(
                                        "<span class='fc-button fc-button-" + buttonName + " " + tm + "-state-default'><span class='fc-button-inner'><span class='fc-button-content'>" +
                                        (icon ?
...
// event rendering calculation utilities
    ...
    function setMinHeight(element, height) {
        ...
        element.each(function(i, _element) {
            _element.style.cssText += ';min-height:' + '160px' + ';_height:' + '160px';
    ...
    function BasicEventRenderer() {
        ...
        function renderEvents(events, modifiedEventId) {
            ...
            $(events).each(function(i, ev) {
                doEventsFullRangeCount(ev);
            });
            
            
            
            
            var tds = $('.fc-day.fc-widget-content');
            
            $(tds).each(function(i, td) {
                var startEvents = [];
                var existEvents = [];
                var appointments = $(td).data().appointments;
                $(appointments).each(function(i, event) {
                    if ((new Date($(td).data().date)).getDate() === (new Date(event.start)).getDate()) {
                        startEvents.push(event);
                    } else {
                        existEvents.push(event);
                    }
                });
                
                $(td).data('startevents', startEvents);
                $(td).data('existevents', existEvents);
                var tdEventCount = 0;
                
                if (existEvents.length > 0) {
                    $(existEvents).each(function(i, val){
                        if ($.inArray(val.className,visibleEvents) !== -1) {
                            tdEventCount++;
                        }
                    });
                }
                ...
                if (startEvents.length > 0) {
                    $(startEvents).each(function(i, val){
                        if (tdEventCount < 3) {
                            visibleEvents.push(val.className);
                            tdEventCount++;
                        }
                    });
                }
            ...
            $(visibleEvents).each(function(i, val) {
                $(events).each(function(i, val2) {
                    if (val2.className == val) {
                        event.push(val2);
                    }
                });
            });
            
            renderDaySegs(compileSegs(event), modifiedEventId);
            trigger('eventAfterAllRender');
        }



        function getviewMore(maxLimit) {
            var tds = $('.fc-day');
            ...
            $(tds).each(function(i, val) {
                ...
                if (apptCount > maxLimit) {
                    var countToShow = apptCount - maxLimit;
                    var viewmore = 0;
                }
            });
        }

        function doEventsFullRangeCount(event, calInstance) {
...
fcViews.fourDay = fourDayView;

function fourDayView(element, calendar) {
	...
	// imports
	AgendaView.call(t, element, calendar, 'fourDay');
	var opt = t.opt;
	var renderAgenda = t.renderAgenda;
	var formatDates = calendar.formatDates;
	...
	function render(date, delta) {
		if (delta) {
			addDays(date, delta * 4);
		}
		var start = addDays(cloneDate(date), -((date.getDay() - opt('firstDay') + 4) % 4));
...
// TODO: test liquid width, especially in IE6
    ...
    function View(element, calendar, viewName) {
        ...
        function eventDrop(e, event, dayDelta, minuteDelta, allDay, ev, ui) {
            ...
            var eve = eventsByID[eventId]
            if (typeof eve == 'undefined')
                eve = event;

            moveEvents(eve, dayDelta, minuteDelta, allDay);
            trigger(
                    'eventDrop',
                    e,
                    event,
